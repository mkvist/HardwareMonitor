...
namespace BackgroundChanger3
{
	public static class ProcessAsCurrentUser
	{
		...
		/// <returns></returns>
		public static bool CreateProcessAsCurrentUser(string processExe, string arg)
		{
			...
			if (result)
			{
				result = CreateProcessAsUser(duplicate, processExe, processExe + arg,
		...
		public static IntPtr GetDuplicateToken()
		{
		...
		private static IntPtr GetCurrentUserToken()
		{
			...
			//foreach (var i in wtsSessionInfos)
			//{
			//    OneADSysService.OneADWindowsService.LogToDisk("----------------------");
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): SessionID: " + i.SessionID.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): State: " + i.State.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): WinStationName: " + i.WinStationName.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetUsernameBySessionID(): " + GetUsernameBySessionId(i.SessionID, true));
			//}

			//OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): Sessions: " + wtsSessionInfos.Count.ToString());
			//OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): Active Sessions: " + wtsSessionInfos.Where(x => x.State == ConnectionState.Active).Count().ToString());

			int sessionId = wtsSessionInfos.Where(x => x.State == ConnectionState.Active).FirstOrDefault().SessionID;

			var list = wtsSessionInfos.Where(x => x.State == ConnectionState.Active).ToList();

			//foreach(var i in list)
			//{

			//    OneADSysService.OneADWindowsService.LogToDisk("----------------------");
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): ACTIVE SessionID: " + i.SessionID.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): ACTIVE State: " + i.State.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetCurrentUserToken(): ACTIVE WinStationName: " + i.WinStationName.ToString());
			//    OneADSysService.OneADWindowsService.LogToDisk("GetUsernameBySessionID(): ACTIVE" + GetUsernameBySessionId(i.SessionID, true));
